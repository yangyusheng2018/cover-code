...
| POST | `/api/branch-coverages/coverage-manual-marks` | `branch-coverage:update` | 详情弹窗人工标记。请求体 **`branchCoverageId`**、**`reportId`**、**`items`**：`{ path, fileMark?, lineMarks? }[]`。`fileMark`：`redundant_covered`（整文件视为已覆盖）或 `instrument_excluded`（整文件从插桩统计中排除）；`lineMarks`：`{ "行号": 同上枚举 }`。`fileMark: null` 或某行值为 `null` 可清除对应标记。合并规则见库表 `coverage_file.manual_marks` 与 `line_details_core`（基准行）+ 再次上报时按基准重放标记、**不参与**父提交行合并。 |
