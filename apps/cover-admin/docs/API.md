...
**跨提交行对齐（远端仓库两提交）**：当父提交 SHA 与当前 `X-Git-Commit` 不同时，服务端请求 GitHub **`repos/{owner}/{repo}/compare/{parent}...{current}`**（两 **commit SHA**）取各文件 unified `patch`。继承规则：根据 patch 与 `@@` 锚点构造 **新行号 → 父行号** 映射（含 **hunk 之间**、首个 hunk 之前与最后一个 hunk 之后与父文件 **1:1 同步** 的未改段，避免「仅在上方插入」导致下方行号整体下移却仍按同号查父而误丢继承）。**`+` 行**以及 **下一 hunk 锚点之前多出来的新行（纯插入、patch 正文未必逐行列出）** 不继承父覆盖；其余行仅在父映射行曾为已覆盖时记到 **当前新行号**（`line_details[].line`）。compare 中**无该文件** patch 时视为该文件未改，按同号对齐继承。compare **失败**时不做跨提交父合并。同提交粘性合并仍有效。仍可用 **`meta.fileChanges.*.resetLines`** 指定不重算的行。
...
| POST | `/api/branch-coverages/coverage-manual-marks` | `branch-coverage:update` | 详情弹窗人工标记。请求体 **`branchCoverageId`**、**`reportId`**、**`items`**：`{ path, fileMark?, lineMarks? }[]`。`fileMark` / 行值枚举：`redundant_covered`（整文件/行视为已覆盖）、`fallback_covered`（兜底代码，与冗余一致计入覆盖）、`instrument_excluded`（从插桩统计中排除）；`lineMarks`：`{ "行号": 同上枚举 }`。`fileMark: null` 或某行值为 `null` 可清除对应标记。合并规则见库表 `coverage_file.manual_marks` 与 `line_details_core`（基准行）+ 再次上报时按基准重放标记、**不参与**父提交行合并。 |
